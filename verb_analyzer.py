...
# --- BEGIN: API for analyze_app.py ---
def validate_prakrit_characters(text):
    """Validate if the input text is a valid Prakrit verb form (forbidden chars, basic phonology)."""
    # Convert to HK for uniformity
    script = detect_script(text)
    if script == 'devanagari':
        hk_text = transliterate(text, 'devanagari', 'hk')
    else:
        hk_text = text
    # Forbidden characters (visarga, retroflex l, etc.)
    forbidden = ['R', 'RR', 'lR', 'lRR', 'H', 'S']
    for char in forbidden:
        if char in hk_text:
            return False, f"The character '{char}' is not found in Prakrit."
    if 'H' in hk_text:
        return False, "The visarga (ः/H) is not found in Prakrit."
    # Basic phonological rules
    if not is_valid_prakrit_sequence(hk_text):
        return False, "The sequence does not follow Prakrit phonological rules."
    return True, ""

def analyze_verb_form(verb_form):
    """Analyze a Prakrit verb form and return a list of possible analyses (as dicts)."""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        import os
        port = int(os.environ.get("PORT", 5000))  # Using a different port from the generator
